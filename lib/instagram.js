...
function igstalk(username) {
    return new Promise(async (resolve, reject) => {
        let host = 'https://cdn1.mystalk.net'
        let form = { data: { 'url': url, 'token': (await _token(host)) } }
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data == 'error') return resolve({ creator: '@pangeran', status: false, message: 'tida dapat diunduh' })
                resolve({ creator: '@pangeran', status: true, thumb: data.thumbnail, data: data.links })
            })
    })
}
